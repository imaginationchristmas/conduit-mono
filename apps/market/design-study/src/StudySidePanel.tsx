...
// The filter controls, rendered without any rail/accordion chrome. This is the
// shared body: the desktop rail (StudySidePanel) wraps it in the foldable
// framed box, and the mobile filters dialog (StudyFiltersDialog) renders the
// same body inside a centered Dialog. Extracting it keeps the two surfaces in
// sync — one copy of the filter logic, two containers.
export function StudyFiltersBody({
  category,
  categories,
  onCategory,
  store,
  ...
  return (
    <>
      ...
      <div className="mt-3">
        <label htmlFor="category-filter" className="study-label mb-1 block">
          Category
        </label>
        <Combobox
          id="category-filter"
      ...
      <div className="mt-3">
        <label htmlFor="store-filter" className="study-label mb-1 block">
          Shop
        </label>
...
//
// Desktop rail chrome is unchanged: it renders the shared StudyFiltersBody
// inside the existing accordion box.
export function StudySidePanel(props: Props) {
